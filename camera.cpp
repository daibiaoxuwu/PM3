...
const int PM_MAX_THREADS = 5;
const std::string STD_ALGORITHM = "PM";
...
void Camera::DofEmit(double i, double j, Vector3* dof_O, Vector3* dof_V) {
	...
	*dof_O = O + Dx /(lens_W * aperture * x )+ Dy/(lens_H * aperture * y);
	*dof_V =  O + Emit(i, j) * focalLen - *dof_O;
}
