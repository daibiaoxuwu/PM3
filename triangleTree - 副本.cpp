...
double TriangleBox::Collide(Ray ray) {
	double minDist = -1;
	for (int coord = 0; coord < 3; coord++) {
		double times = -1;
		...
		if (times >= EPS) {
			Vector3 C =ray.pos + ray.dir * times;
			if (Cantain(C)) {
				double dist =ray.pos.Distance(C);
...
Collider TriangleTree::TravelTree(TriangleNode* node, Ray ray) {
	if (!node->box.Cantain(ray.pos) && node->box.Collide(ray) <= -EPS)
		return Collider();

	if (node->leftNode == NULL && node->rightNode == NULL) {
		Collider ret;
		for (int i = 0; i < node->size; i++) {
			Collider collider = node->tris[i]->Collide(ray);
	...
	if (node->leftNode->box.Cantain(ray.pos)) {
		Collider collider = TravelTree(node->leftNode, ray);
		if (collider.crash) return collider;
		return TravelTree(node->rightNode, ray);
	}
	if (node->rightNode->box.Cantain(ray.pos)) {
		Collider collider = TravelTree(node->rightNode, ray);
		if (collider.crash) return collider;
		return TravelTree(node->leftNode, ray);
	}

	double leftDist = node->leftNode->box.Collide(ray);
	double rightDist = node->rightNode->box.Collide(ray);
	if (rightDist <= -EPS)
		return TravelTree(node->leftNode, ray);
	if (leftDist <= -EPS)
		return TravelTree(node->rightNode, ray);
...
Collider TriangleTree::Collide(Ray ray) {
